...
# ---------------------------------------------------------------------------
# (extra_dx, extra_dy, extra_rot_deg, glow_intensity)  for each frame of
# the attack anim. The weapon's body-lockstep pose gets these motions
# applied on top:
#   - rotation around WEAPON_ROT_PIVOT[piece] by extra_rot_deg
#   - then translation by (extra_dx, extra_dy)
# Then the glow paints a small bright cluster at the weapon's top
# opaque pixel with the given intensity (0..1).
#
# For staff pieces (bishop/king/queen) the rotation gives the staff its
# OWN motion that's distinct from the body's lean — staff "cocks back"
# while the body lunges forward, reading as two coordinated sprites
# rather than one welded silhouette. Rotation also keeps the orb in
# canvas (a pure -12 px translate would clip the orb off the top).

WEAPON_EXTRAS = {
    # Staff pieces (bishop/king/queen): NO sprite-level motion or glow
    # in weapon_attack.png. Position/rotation changes during the attack
    # are handled in Godot (anticipate/lunge/settle tween of the
    # floating piece). The visual interest of the attack comes from
    # the SEPARATE flash sprite (weapon_flash_attack.png) overlaid at
    # runtime — see _render_staff_flash.py.
    "bishop": {
        "attack": [(0, 0, 0, 0.00)] * 6,
    },
    "king": {
...
# Rotation pivot per piece (canvas coords on the 64×64 weapon sprite).
# Only weapons with non-zero extra_rot need an entry here — typically
# the wielder's grip position so the weapon swings/cocks naturally.
WEAPON_ROT_PIVOT = {
    "bishop": (44, 38),
    "king":   (44, 38),
    "queen":  (42, 38),
...
# Note: the magic-cast flash that emanates from the staff orb is NOT
# baked into weapon_attack.png. It lives as a separate sprite strip
# (weapon_flash_attack.png, generated by _render_staff_flash.py) and
# is composited at runtime as a Godot overlay — see §5j of
# THEME-WIZARDS-GUILD.md. Putting it on its own canvas means the burst
# can extend beyond the 64×64 weapon canvas without being clipped, and
# the staff sprite stays clean of FX.
...
def split_anim(piece: str, color: str, anim: str) -> dict:
    ...
    for i, (dx, dy, squash, lean_top) in enumerate(poses):
        ...
        if extras is not None and i < len(extras):
            ex_dx, ex_dy, ex_rot, glow_int = extras[i]
            # Rotate first (around the wielder's grip), then translate.
            # Rotation gives the weapon its own motion — for staffs this
            # is a back-cock during charge + forward swing on discharge,
            # decoupled from the body's lean (so they read as two
            # coordinated sprites rather than one welded silhouette).
            if ex_rot != 0 and piece in WEAPON_ROT_PIVOT:
                weapon_frame = rotate_around(
                    weapon_frame, ex_rot, WEAPON_ROT_PIVOT[piece])
            if ex_dx or ex_dy:
        ...
        # NOTE: aura rings are NOT baked into the weapon_attack.png —
        # they live as a separate Godot-side procedural effect (see
        # AuraRing.gd / _spawn_staff_aura in GameScene.gd) so the rings
        # can extend any size without being clipped by the 64×64 canvas
        # and so the staff sprite isn't conflated with its FX.
