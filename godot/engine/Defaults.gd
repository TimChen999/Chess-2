## Defaults factory — builds the standard six piece defs and an opening setup.
## Called by GameSettings on first run (no save file present). Tunable values
## documented in IMPL-GODOT §10.6 and DESIGN.md §10/§11.
class_name Defaults
extends RefCounted
...
static func _make_special(kind: int, damage: int = 1, cd: int = 3,
...
static func _make_piece(id: String, name: String, glyph: String,
                        hp: int, dmg: int, royal: bool, can_castle: bool,
                        patterns: Array, on_hit: StatusEffectDef = null,
                        special: SpecialAbilityDef = null,
    ...
    d.special = special if special != null else _make_special(SpecialAbilityDef.Kind.NONE)
    d.promotes_to = promotes_to
    if promotes_to.size() > 0:
        d.promotes_at_rank = -1     ## emitted by pawn pattern; rank handled there
    return d
...
static func make_default_config() -> GameConfig:
    ...
    cfg.pieces["queen"] = _make_piece(
        ...
        null,
        _make_special(SpecialAbilityDef.Kind.CANNON, 2, 4, 1, 0),
    )
    ...
    cfg.pieces["bishop"] = _make_piece(
        ...
        [_make_pat(MovePattern.Kind.RIDER, DIAG)],
        null,
        _make_special(SpecialAbilityDef.Kind.LIGHTNING, 1, 3, 1, 1),
    )
